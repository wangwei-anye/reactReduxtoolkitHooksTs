...
const calculatePlygon = (x, y, radian = 0, length = 4.5, width = 1.9) => {
...
async function buildXvizData(fileUrl) {
  ...
  const json = await axios.get('http://192.168.113.109:8080/download/' + fileUrl);
  ...
  obstacleCar.pos.forEach((element, index) => {
    ...
    xvizBuilder.primitive(VEHICLE_TRAJECTORY).polyline(line).id('object-2').style({
      stroke_width: 0.5
    });
